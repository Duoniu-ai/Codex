...
## 5.1 现状
- MySQL 127.0.0.1:3306，库 `ipquery`，**20 张表**（PHP 43 主库数据仓库：ip_record/asn/company/location/abuse/evidence/score_cache/query_log/visitor_ips/ip_history(字段级日志)/ip_segment*/ip_asn_segment/ip_type_segment*/blacklist_events/collector_state/provider_status/whois_raw/v_ip_detail + webrtc_peers，写入已修复 2026-08-15）
- 文件存储：data/location-history.json（月度快照）、.cache/asn-list.json

## 5.2 兼容步骤
1. ~~修复凭据~~（**已完成 2026-08-15 Phase 0**：PHP 主库同库凭据 → .env（0600）→ systemd EnvironmentFile → 重启验证连通，webrtc_peers 27 行）
2. 迁移脚本 `scripts/migrate-history.mjs`（幂等）：把 location-history.json 的每条记录 upsert 进 `ip_location_history`，字段对齐后 JSON 文件保留为只读备份
3. **现有 20 张表盘点**（Phase 1 前置）：PHP 主库已有 ip_record/asn/company/location/ip_history(字段级日志)/score_cache 等——Adapter 层设计前需摸清哪些表可直接读用、避免重复建表
...
-- IP 位置历史（承接 location-history.json）
-- 注意：ip_history 名已被 PHP 主库占用（字段级变更日志 ip/field/old_value/new_value），勿重名
