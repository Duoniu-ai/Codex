# 网址发现平台 — 数据库 Schema 与数据同步契约 V1.0（候选版）
...
> 版本:V1.0-rev1(2026-08-17 外部审计 PASS WITH 4 FIXES,修订 A1/A2/A3/A4,修订记录见文末)。状态:**候选版,等待外部审计后再冻结**。所有表/字段/枚举/契约规则在本文档中即为最终定义依据。
...
3. **状态**:候选版,等待外部审计后再冻结;不执行 Prisma migrate / schema.prisma / API 开发。
...
| 6 | 架构 V1.0-rev1 同步 task_aliases / task_related_tags / website_changes 字段级描述 | 待 Schema 冻结后回写 |
...
> 状态:**仍为候选版,等待外部审计复核后再冻结**;未执行 Prisma migrate / schema.prisma / API 开发。
