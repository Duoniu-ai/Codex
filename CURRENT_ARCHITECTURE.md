...
- **MySQL** 127.0.0.1:3306，库 `ipquery`，共 **20 张表**——PHP 43 主库的数据仓库：ip_record（主记录，全部 is_* 风控标志）/ asn / company / location / abuse / evidence / score_cache / query_log / visitor_ips / **ip_history（字段级变更日志：ip/field/old_value/new_value/source/observed_at）** / ip_asn_segment / ip_segment / ip_type_segment(_v6) / blacklist_events / collector_state / provider_status / whois_raw / v_ip_detail（视图）+ webrtc_peers（Nuxt peer 采集，**现存 27 行**）。**2026-08-15 Phase 0 已修复写入**：DB_PASS 提取自 PHP 主库 config.php → duoniu-new-vue/.env（0600）→ systemd `duoniu-preview` 加 EnvironmentFile，重启后连通验证通过
...
> **2026-08-15 Phase 0 勘误/进展**：问题 #2 已修复（DB 凭据接入，落库恢复）；Phase 0 完成项：git 基线 d0c9265、备份 /www/backup/duoniu-phase0-20260815/、staging 环境（duoniu-staging:3902 + staging.duoniu.cc 域名）、deploy/rollback/smoke 脚本、19 个 .bak 与 20 个 tar.gz 归档、孤儿 build 进程已清理。
...
2. ~~**webrtc_peers 落库失败**~~（**已修复 2026-08-15**）：原无 .env、systemd 单元未配置 DB_PASS → nohup.out 出现 6 条 `[peer] store failed`。已提取 PHP 主库同库凭据写入 .env（0600）+ EnvironmentFile 接线 + 重启验证通过，webrtc_peers 现 27 行。
