# 网址发现平台 — API 契约设计 V1.0(冻结版)

> **【冻结声明】本文件为"网址发现平台"API 契约的正式冻结版本(2026-08-17 正式冻结)。冻结范围至少包括(30 项):**
> 1. Public API 接口契约;2. Admin API 接口契约;3. SiteIntel 上游消费契约;4. 搜索接口行为契约;5. normalized_domain 规范化规则;6. Task / Alias / Tag 搜索语义;7. task_related_tags 辅助匹配边界;8. 网站详情数据返回边界;9. 网站关系 similar / alternative P0 allowlist;10. related P1 门控;11. recommendations 稳定定义层与实时执行层隔离;12. submissions 幂等与限流;13. reports 匿名纠错与字段白名单;14. events visitor_id 服务端生成规则;15. 收藏纯前端 localStorage / IndexedDB;16. ADMIN_EMAILS 单角色后台权限;17. SiteIntel navigation-sync API Key 边界;18. 上游字段白名单与禁止持久化规则;19. summary_hash 固定 12 字段规则;20. sync_queue 状态机(pending/processing/completed/retrying/failed/skipped 六枚举,以 Schema 冻结版 §2.21 CHECK 为准);21. CAS 原子领取;22. Worker Lock;23. 10 分钟超时恢复;24. attempts 先 +1 再判断;25. attempts >= max_attempts 即 failed;26. 默认 max_attempts = 5;27. 第 5 次失败即 failed,无第 6 次;28. 指数退避;29. 幂等与断点续跑;30. P0 / P1 功能边界。
> **冻结后任何对上述范围的修改不得直接修改本文件,必须走"API Change Request → 影响分析 → Schema Change Request 判定 → 新版本号 → 内部一致性检查 → 外部/专项审计 → 新冻结版"流程(见文末"冻结后变更规则")。**

> 版本:V1.0(2026-08-17 正式冻结)。状态:**已冻结**。前置:外部冻结审计 PASS WITH 5 FIXES(P1-1~P1-5 修复 + P2-9/P2-10 零成本修正)→ 内部一致性检查重跑 20/20 PASS + 修复后专项检查 S1-S7 PASS(见《docs/audit/API契约冻结验收记录-V1.0.md》)。**API 开发以本文件为唯一依据。**
> 依据基线:①《网址发现平台-完整产品规划-V2.1.2-最终冻结版》;②《网址发现平台-数据库Schema与数据同步契约-V1.0-冻结版》(24 表);③《网址发现平台-技术架构设计-V1.0-候选版》(V1.0-rev2,未冻结)。
> 内部一致性检查见《docs/audit/API契约设计-内部一致性检查报告.md》;冻结审计见《docs/audit/API契约冻结审计报告-V1.0.md》。
...
3. **状态**:已冻结(V1.0,2026-08-17);本阶段不写 schema.prisma / route.ts / Worker / 前端,进入前后台信息架构阶段。
...
- 鉴权规则;
- 限流规则;
- normalized_domain 规则;
- P0/P1 功能边界;
- related 门控;
- recommendations 落库边界;
- 收藏本地化规则;
- SiteIntel 数据消费边界;
- summary_hash 规则;
- sync_queue 状态机及 attempts 边界。

如果未来发现问题,不得直接修改冻结版,必须执行:

1. **API Change Request**(理由 + 预期影响);
2. **影响分析**(涉及接口 / 数据表 / 上下游);
3. **判断是否涉及 Schema Change Request**(涉及则按《数据库Schema与数据同步契约-V1.0-冻结版》文末"冻结后变更规则"另行流转,不直接改 Schema 冻结版);
4. **新版本号**(如 V1.1 候选版);
5. **内部一致性检查**;
6. **外部/专项审计**;
7. **生成新的冻结版**(旧版本归档保留)。
